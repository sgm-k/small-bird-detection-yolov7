...
import os, sys, re, click, json, requests
from typing import List
from pycocotools.coco import COCO

from coco2yolo.submodules.FileSearcher import get_filenames, check2create_dir
from coco2yolo.submodules.WordOperator import str_format, ask_yn
    ...
    def _create_symbolic_links(self):
        ...
        for img_info in self.exist_img_infos:
            try:
                ...
                print(self.img_dir+"/"+img_info["file_name"])
                print(self.task_dir+"/"+img_info["file_name"])
                img = cv2.imread(self.img_dir+"/"+img_info["file_name"])
                img_info["file_name"]=img_info["file_name"].replace("/","-")
                cv2.imwrite(self.task_dir+"/"+img_info["file_name"],img)
